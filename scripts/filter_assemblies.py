...
parser.add_argument("--original_path", type=str, default='/home/gomez/gomez/assemblies_linkbert_500',
    ...
    if len(assemblies) > max_assemblies:
        ...
        for assembly in selected_assemblies:
            ...
            if os.path.getsize(glob(f"{assembly_path}/*.fna.gz")[0]) < 5120:
                available_assemblies = [a for a in assemblies if os.path.getsize(
                    glob(f"{original_path}/{strain}/{a}/*.fna.gz")[0]) >= 5120]
                if available_assemblies:
                    ...
                    continue
            shutil.copytree(
                assembly_path, f"{filtered_path}/{strain}/{assembly}",dirs_exist_ok = True)
    else:
